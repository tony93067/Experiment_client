...
char transmit_with_method[15] = {0};
char cc_method[15] = {0};
...
void* timer(void* arg)
{
    ...
    while(1)
    {
        ...
        if(t == 300)
        {
            ...
            memset(str, '\0', sizeof(str));
            sprintf(str, "%s\t%s\n", "Transmit with", transmit_with_method);
            write(ex, str, sizeof(str));
...
int main(int argc, char **argv)
{
    ...
    if(argc != 5)
    {
        printf("Usage: %s <server_ip> <Background TCP Number> <congestion control> <second flow congestion control>\n",argv[0]);
        exit(1);
    }
    
    strcpy(cc_method, argv[3]);
    strcpy(transmit_with_method, argv[4]);
